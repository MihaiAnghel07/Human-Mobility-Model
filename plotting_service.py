...
def load_node_positions(file_path):
...
def plot_node_movements(node_positions, context_data=None):
    size = (50, 50)
    if context_data:
    ...
    plt.title('Context: Obstacles, Homes, Pubs, Others')
    plt.xlim(0, size[0])
    plt.ylim(0, size[1])
    plt.xlabel('X')
    plt.ylabel('Y')
...
def draw_heatmap_from_paths(node_positions, size):
    ...
    # adjust contrast to increase visibility of traffic
...
if __name__ == "__main__":
    ...
    context_path = 'input/input_large.txt'
    
    node_positions = load_node_positions(output_path)
    context_data = read_context_file(context_path)
    
    plot_node_movements(node_positions, context_data)
